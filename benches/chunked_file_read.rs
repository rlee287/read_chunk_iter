...
fn do_std_iter_bufread(file: File, chunk_size: usize) {
    ...
    for chunk in &bufread.bytes().chunks(chunk_size) {
        assert_eq!(chunk.map(|x| x.unwrap()).collect::<Vec<_>>(), vec![0xf0; chunk_size]);
    }
}
...
fn criterion_benchmark_multiplier(c: &mut Criterion) {
    ...
    let mut group: criterion::BenchmarkGroup<'_, criterion::measurement::WallTime> = c.benchmark_group("(verify, filesize:2MiB, chunk:8192, multiplier:?)");

    for multiplier in [1,2,4,6,8] {
        group.bench_with_input(BenchmarkId::new("simple", multiplier),
            &multiplier, |b, &mult| {
            b.iter_batched(|| File::open(temp_file_path).unwrap(),
                |file| do_chunked_read(file, black_box(8192), mult),
                criterion::BatchSize::PerIteration);
        });
        group.bench_with_input(BenchmarkId::new("threaded", multiplier),
            &multiplier, |b, &mult| {
            b.iter_batched(|| File::open(temp_file_path).unwrap(),
...
fn criterion_benchmark_filesize(c: &mut Criterion) {
    ...
    for file_size in [128, 256, 512, 1024, 1024*2] {
        ...
        group.bench_with_input(BenchmarkId::new("simple", file_size),
            &file_size, |b, &_size| {
            b.iter_batched(|| File::open(temp_file_path).unwrap(),
                |file| do_chunked_read(file, black_box(16384), black_box(4)),
                criterion::BatchSize::PerIteration);
        });
        group.bench_with_input(BenchmarkId::new("threaded", file_size),
            &file_size, |b, &_size| {
            b.iter_batched(|| File::open(temp_file_path).unwrap(),
...
fn criterion_benchmark_chunksize(c: &mut Criterion) {
    ...
    for chunk_size in [1024*2, 1024*4, 1024*8, 1024*16, 1024*32] {
        ...
        group.bench_with_input(BenchmarkId::new("iter_bufread", chunk_size),
            &chunk_size, |b, &size| {
            b.iter_batched(|| File::open(temp_file_path).unwrap(),
                |file| do_std_iter_bufread(file, size*4),
                criterion::BatchSize::PerIteration);
        });
        group.bench_with_input(BenchmarkId::new("simple", chunk_size),
            &chunk_size, |b, &size| {
            b.iter_batched(|| File::open(temp_file_path).unwrap(),
                |file| do_chunked_read(file, size, black_box(4)),
                criterion::BatchSize::PerIteration);
        });
        group.bench_with_input(BenchmarkId::new("threaded", chunk_size),
            &chunk_size, |b, &size| {
            b.iter_batched(|| File::open(temp_file_path).unwrap(),
